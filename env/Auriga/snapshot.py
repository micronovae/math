...
def loadSubset(basePath, snapNum, partType, fields=None, subset=None, mdi=None, sq=False):
    ...
    with h5py.File(snapPath(basePath, snapNum), 'r') as f:
        ...
        if not numToRead:
            ...
            f = h5py.File(snapPath(basePath, snapNum,), 'r')
            for i, field in enumerate(fields):
                # assign empty array to reslut
                if mdi is None or mdi[i] is None:
                    result[field] = f[gName][field][0:0]
                else:
                    result[field] = f[gName][field][0:0, mdi[i]]
            f.close()
            return result
